...
import java.lang.*;
public class monster4 extends GameManager implements Runnable{
	static ImageIcon monster4;
	...
	public monster4(String ptr){
		monster4=new ImageIcon(ptr);
		monsterIcon=new JLabel(monster4);
	...
	public void monsterMove(){
		for(int i=0,count=0;i<howMuchTime;i++){
			...
			if(RightOrLeft == 1 &&monsterIcon.getX()+movedist<1340){
				monster4=new ImageIcon("image\\monster\\ms.right.png");
				monsterIcon.setIcon(monster4);
				monsterIcon.setLocation(monsterIcon.getX()+movedist,monsterIcon.getY());
			}
			else if(RightOrLeft == 0 &&monsterIcon.getX()-movedist>0){
				monster4=new ImageIcon("image\\monster\\ms.left.png");
				monsterIcon.setIcon(monster4);
				monsterIcon.setLocation(monsterIcon.getX()-movedist,monsterIcon.getY());
			}
			try {
			...
			//System.out.println("monster4 " + monsterIcon.getX() + " " + monsterIcon.getY());
		}
	}
	...
	public void monsterDieAndReborn(){
		if(health==1){ //death 		
			...
			*/
			...
			msh.start();
			if(RightOrLeft == 1) { monster4=new ImageIcon("image\\monster\\mdie2.gif"); }
			else{ monster4=new ImageIcon("image\\monster\\mdie1.gif"); }
			monsterIcon.setIcon(monster4);
			try { 
				Thread.sleep(80); 
			} 
			...
			if(RightOrLeft == 1) {monster4=new ImageIcon("image\\monster\\ms.right.png");}
			else{ monster4=new ImageIcon("image\\monster\\ms.left.png"); }
			monsterIcon.setIcon(monster4);
			health=3;

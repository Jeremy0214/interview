...
import java.lang.*;
public class monster2 extends GameManager implements Runnable{
	static ImageIcon monster2;
	...
	public monster2(String ptr){
		monster2=new ImageIcon(ptr);
		monsterIcon=new JLabel(monster2);
	...
	public void monsterMove(){
		for(int i=0,count=0;i<howMuchTime;i++){
			...
			if(RightOrLeft == 1 &&monsterIcon.getX()+movedist<1340){
				monster2=new ImageIcon("image\\monster\\ms.right.png");
				monsterIcon.setIcon(monster2);
				monsterIcon.setLocation(monsterIcon.getX()+movedist,monsterIcon.getY());
			}
			else if(RightOrLeft == 0 &&monsterIcon.getX()-movedist>0){
				monster2=new ImageIcon("image\\monster\\ms.left.png");
				monsterIcon.setIcon(monster2);
				monsterIcon.setLocation(monsterIcon.getX()-movedist,monsterIcon.getY());
			}
			try {
			...
			//System.out.println("monster2 " + monsterIcon.getX() + " " + monsterIcon.getY());
		}
	}
	...
	public void monsterDieAndReborn(){
		if(health==1){ //death 		
			...
			*/
			...
			msh.start();
			if(RightOrLeft == 1) { monster2=new ImageIcon("image\\monster\\mdie2.gif"); }
			else{ monster2=new ImageIcon("image\\monster\\mdie1.gif"); }
			monsterIcon.setIcon(monster2);
			try { 
				Thread.sleep(80); 
			} 
			...
			if(RightOrLeft == 1) {monster2=new ImageIcon("image\\monster\\ms.right.png");}
			else{ monster2=new ImageIcon("image\\monster\\ms.left.png"); }
			monsterIcon.setIcon(monster2);
			health=3;

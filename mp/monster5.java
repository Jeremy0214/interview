...
import java.lang.*;
public class monster5 extends GameManager implements Runnable{
	static ImageIcon monster5;
	...
	public monster5(String ptr){
		monster5=new ImageIcon(ptr);
		monsterIcon=new JLabel(monster5);
	...
	public void monsterMove(){
		for(int i=0,count=0;i<howMuchTime;i++){
			...
			if(RightOrLeft == 1 &&monsterIcon.getX()+movedist<1340){
				monster5=new ImageIcon("image\\monster\\ms.right.png");
				monsterIcon.setIcon(monster5);
				monsterIcon.setLocation(monsterIcon.getX()+movedist,monsterIcon.getY());
			}
			else if(RightOrLeft == 0 &&monsterIcon.getX()-movedist>0){
				monster5=new ImageIcon("image\\monster\\ms.left.png");
				monsterIcon.setIcon(monster5);
				monsterIcon.setLocation(monsterIcon.getX()-movedist,monsterIcon.getY());
			}
			try {
			...
			//System.out.println("monster5 " + monsterIcon.getX() + " " + monsterIcon.getY());
		}
	}
	...
	public void monsterDieAndReborn(){
		if(health==1){ //death 		
			...
			*/
			...
			msh.start();
			if(RightOrLeft == 1) { monster5=new ImageIcon("image\\monster\\mdie2.gif"); }
			else{ monster5=new ImageIcon("image\\monster\\mdie1.gif"); }
			monsterIcon.setIcon(monster5);
			try { 
				Thread.sleep(80); 
			} 
			...
			if(RightOrLeft == 1) {monster5=new ImageIcon("image\\monster\\ms.right.png");}
			else{ monster5=new ImageIcon("image\\monster\\ms.left.png"); }
			monsterIcon.setIcon(monster5);
			health=3;
